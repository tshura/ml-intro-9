    ...
    with mlflow.start_run():
        ...
        if m not in ["logreg", "rf"]:
            raise click.BadParameter(
                "model takes values 'logreg' for LogisticRegression or 'rf' for RandomForestClassifier"
            )
        if s not in ["ss", "mm"]:
            raise click.BadParameter(
                "scaler takes values 'ss' for StandardScaler or 'mm' for MinMaxScaler"
            )
        ...
        if use_ncv is False:
            if use_tsr is not False:
                ...
                scores = cross_validate(
                    pipeline, features_train, target_train, cv=foldcnt, scoring=scoring
                )
                for i in scoring:
